    ...
    'configure':{
        'profile': {
            ...
            'set': {
                'fail':'You need to specify a key and a value. See `{}help profile set` for more',
                'colour': {
                    'help': '''Set the color for embeds and banners.
                               Provide a hex code like `[p]profile set color #03dc03` or `[p]profile set color 0088ff` or even `[p]profile set color 0x123456`
                               For the default color, do:
                               [p]profile set color #bb1177''',
                    'set': 'Your profile color has been set.',
                    'invalid': 'That color code is invalid.',
                },
                'username': {
                    'help': '''Set your osu! username.
                               Also sets the default user for [p]user and [p]banner.
...
def get_locale(locale, string):
    strings = {'en':en,'us':us}[locale] # add more to this dict when more locales
    path = string.split('.')
    for i in path:
        strings = strings[i]
    return strings